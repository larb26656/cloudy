...
| `chatPanelStore.ts`                             | none                                 | Per-tab chat UI state — `filesOpenByTabId`. Read by `ChatContent` (desktop side panel + mobile bottom sheet) and `ChatHeaderActions`; keyed by tabId for independent tabs.                     |
