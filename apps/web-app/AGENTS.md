...
**schema versioning** (`version: 7`). When changing the `Tab` shape, bump the version and
...
null `workspaceId`), v6→v7 (remove `workspaceId` from terminal tabs).
...
| `chatPanelStore.ts`                             | localStorage "chat-panel" v1 (widths only, via `partialize`) | Per-tab chat UI state — `filesOpenByTabId` (ephemeral) + `filesWidthByTabId` (persisted files-panel width, set from `ResizablePanelGroup.onLayoutChanged`). Cleaned up by chat tab `onClose` (`clearTab`) and pruned against live tab ids on boot in `main.tsx` (`pruneExcept`). Read by `ChatContent` (desktop side panel + mobile bottom sheet) and `ChatHeaderActions`; keyed by tabId for independent tabs. |
| `chatInputHistoryStore.ts`                      | localStorage "chat-input-history" v1                         | Per-session prompt history (string[] map keyed by sessionId). Pushed by `ChatProvider.sendMessage`; read by `ChatInput` for ArrowUp/ArrowDown recall. Dedupes consecutive entries, caps at 50.                                                                                                                                                                                                                  |
| `recentDirectoryStore.ts`                       | localStorage "recent-directories" v1                         | Recent quick-chat directory paths (dedupe, move-to-front, cap 8, per-item remove). Pushed/read by `CreateChatDialog`'s `QuickPathSection`.                                                                                                                                                                                                                                                                      |
| `defaultAgentStore.ts` / `defaultModelStore.ts` | —                                                            | Default agent/model pickers.                                                                                                                                                                                                                                                                                                                                                                                    |
| `favoriteModelsStore.ts`                        | `localStorage` "favorite-models" v1                          | `ModelConfig[]` (LIFO) of pinned models shown at the top of `ModelSelector`. Stale entries (model gone from the opencode provider list) are filtered at render time, not deleted — they can come back if the provider returns.                                                                                                                                                                                  |
...
- **Icons:** `lucide-react` only. The `data-icon` attribute on `<Icon>` elements is a
  ...
  **Exception:** per-extension file-type icons (the `FileTree`) use
  `@iconify/react` + a curated subset of `@iconify-json/vscode-icons` so files
  get colored VSCode-style glyphs. The mapping lives in
  `src/lib/file-icons.ts`; the bundled icon data is regenerated by
  `pnpm --filter web-app generate:file-icons` into
  `src/lib/file-icons.generated.json` (only the ~50 referenced icons are
  shipped, not the full 1 MB-gzipped collection). Render via
  `<FileTypeIcon name={node.name} />` from `@/components/files/FileTypeIcon`.
