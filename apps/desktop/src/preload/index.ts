...
export interface ElectronContextAPI {
  addContext: (options: { type: string; data: unknown; replace?: boolean }) => Promise<{ status: 'added' | 'replaced'; id: string }>
  removeContext: (id: string) => Promise<boolean>
  clearContexts: () => Promise<boolean>
  listContexts: () => Promise<Array<{ id: string; type: string; data: unknown; replace: boolean; timestamp: string }>>
  onContextUpdate: (
    callback: (event: {
      action: 'added' | 'removed' | 'cleared'
      item?: { id: string; type: string; data: unknown; replace: boolean; timestamp: string }
      contexts: Array<{ id: string; type: string; data: unknown; replace: boolean; timestamp: string }>
    }) => void,
  ) => () => void
}
...
declare global {
  interface Window {
    electronAPI: { context: ElectronContextAPI; server: ElectronServerAPI; config: ElectronConfigAPI }
  }
}