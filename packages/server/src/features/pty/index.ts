...
import type { PtyService } from "./service";
 ...
 * discovery.
 */
export function createPtyApp({ ptyService }: { ptyService: PtyService }) {
  return new Hono()
    .get(
      "/shells",
      describeRoute({
      ...
      (c) => c.json(ptyService.listShells()),
    )
    .post(
      "/sessions",
      describeRoute({
      ...
      async (c) => {
        ...
        const created = ptyService.createSession(input);
    ...
    .get(
      ...
      (c) => {
        ...
        return c.json(ptyService.getSession(id));
      },
    )
    .post(
      "/sessions/:id/resize",
