...
interface TabGroupButtonProps<T extends string = string> extends VariantProps<
  typeof tabGroupButtonVariants
> {
...
function TabGroupButton<T extends string = string>({
  ...
  return (
    <div className={cn("flex items-center overflow-x-auto", className)}>
      <div
        role="tablist"
        className={cn(tabGroupButtonVariants({ orientation }))}
      >
        {options.map((option) => (
          <Button
            key={option.value}
